...
def main(fname):
    ...
    # Populating dictionary and inverseDictionary
    ...
    print("\n\n")
    print("--------Above, 0th Column represents distance vector of A, similarly 1st Column represents distance vector for B and so on...----------")
...
def router(idRouter, rTable, sharedQ, adjRouters, locks, barrier):
    ...
    for i in range(0, iterations):
        ...
        # Used to check if node.routingTable gets updated
        updated = [0] * len(rTable)
        for adj in adjRouters:
            newTable, senderId = sharedQ[idRouter].get(
                True)  # Getting data from queue
        ...
        print(
            "\n---------------Routing table of router {} after ITERATION :{}--------------\n\n{}".format(
                dicti[idRouter],
                i + 1,
